...
def _carregar_variaveis_ambiente_locais() -> None:
    ...
    for nome_arquivo in (".env.local", ".env", ".env.example"):
        ...
        try:
            ...
            logger.warning("Falha ao carregar variáveis locais de %s: %s", caminho, exc)
...
class LLMDecisionConfig:
    ...
    backend_mode: str = field(default_factory=lambda: os.getenv("LLM_BACKEND_MODE", "llama_cpp").strip().lower())
    model_name: str = field(default_factory=lambda: os.getenv("LLM_MODEL_NAME", "qwen2.5:1.5b-instruct"))
    model_repo: str = field(default_factory=lambda: os.getenv("LLM_MODEL_REPO", "Qwen/Qwen2.5-1.5B-Instruct-GGUF"))
    ...
    def _resolve_model_path(self) -> Path:
        ...
        if self.model_path is not None:
            if not self.model_path.exists():
                raise BackendUnavailableError(f"Caminho local do modelo não encontrado: {self.model_path}")
            self._model_path = self.model_path
            return self._model_path

        self.model_cache_dir.mkdir(parents=True, exist_ok=True)
        if importlib.util.find_spec("huggingface_hub") is None:
            raise BackendUnavailableError("Dependência huggingface_hub não está disponível no ambiente.")
        ...
        with self._lock:
            ...
            try:
                if importlib.util.find_spec("llama_cpp") is None:
                    raise BackendUnavailableError("Dependência llama-cpp-python não está disponível no ambiente.")
    ...
    def generate(
        ...
        *,
        ...
        try:
            ...
            future.cancel()
            raise TimeoutError("Inferência local excedeu o tempo limite.") from exc
...
def _create_backend_cached(
    ...
    mode = (backend_mode or "llama_cpp").strip().lower()
    if mode in {"disabled", "off", "false"}:
        return None
    if mode != "llama_cpp":
        raise BackendUnavailableError(f"Modo de backend não suportado nesta versão: {mode}")
...
def create_default_backend(config: Optional[LLMDecisionConfig] = None) -> Optional[LLMBackend]:
    ...
    try:
        ...
        logger.warning("Backend semântico indisponível: %s", exc)
        return None
